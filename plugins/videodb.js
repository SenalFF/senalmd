...
async function sendFromGofile(robin, from, mek, gofileUrl, title) {
  await robin.sendMessage(
    from,
    {
      ...
      mimetype: "video/mp4",
      fileName: `${title.slice(0, 30)}.mp4`,
      caption: "✅ *Sent via Gofile stream*",
    },
    { quoted: mek }
  );
}
...
cmd(
  ...
  async (robin, mek, m, { q, reply }) => {
    ...
    if (!q)
      return reply("🔍 *කරුණාකර වීඩියෝ නමක් හෝ YouTube ලින්ක් එකක් ලබාදෙන්න*");
    try {
  ...
  async (robin, mek, m, { reply }) => {
    ...
    try {
      ...
      if (filesize > MAX_VIDEO_SIZE) {
        await reply("📤 File too large. Uploading to Gofile...");
        const upload = await uploadToGofile(
          buffer,
          `${session.video.title.slice(0, 30)}.mp4`
        );
        if (!upload.success) {
          console.error("Gofile upload failed:", upload.error);
          return reply("❌ *Failed to upload to Gofile.*");
        }
...
cmd(
  ...
  async (robin, mek, m, { reply }) => {
    ...
    try {
      ...
      await reply(`📦 *File size:* ${sizeMB} MB`);
      if (filesize > MAX_VIDEO_SIZE) {
        await reply("☁️ File too large. Uploading to Gofile...");
        const upload = await uploadToGofile(
          buffer,
          `${session.video.title.slice(0, 30)}.mp4`
        );
