...
cmd({
  ...
  try {
    ...
    if (!download_links || Object.keys(download_links).length === 0) {
      return m.reply("⚠️ *No download links available for this movie.*");
    }
...
cmd({
  ...
  try {
    ...
    if (!download_links || Object.keys(download_links).length === 0) {
      return m.reply("⚠️ *No download links available for this TV show.*");
    }
    ...
    console.error("tvshowdl error:", e);
    m.reply("⚠️ An error occurred while executing the command.");
  }
});
