...
async function connectToWA() {
    ...
    sock.ev.on('messages.upsert', async (meks) => {
        ...
        const isBotAdmins = isGroup ? groupAdmins.includes(jidNormalizedUser(sock.user.id)) : false;
        const isAdmins = isGroup ? groupAdmins.includes(sender) : false;

        const reply = (text, extra = {}) => sock.sendMessage(from, { text, ...extra }, { quoted: mek });

        // ================= BUTTON HELPERS =================
        const sendTextButton = async (text, footer, buttons) => {
            const buttonMessage = { text, footer, buttons, headerType: 2 };
            await sock.sendMessage(from, buttonMessage, { quoted: mek });
        };
        ...
        const sendMediaButton = async (type, url, caption, footer, buttons) => {
            ...
            msg.buttons = buttons;
            msg.headerType = 1;
            await sock.sendMessage(from, msg, { quoted: mek });
        };
        ...
        const sendInteractiveMessage = async (text, title, footer, interactiveButtons) => {
            ...
            await sock.sendMessage(from, message, { quoted: mek });
        };

        // ================= COMMAND HANDLER =================
        const events = require('./command');
        const cmdObj = isCmd ? (events.commands.find(c => c.pattern === commandText) || events.commands.find(c => c.alias?.includes(commandText))) : null;
        ...
        if (cmdObj) {
            try {
                await cmdObj.function(sock, mek, m, {
                    ...
                    isBotAdmins, isAdmins, reply, sendTextButton, sendMediaButton, sendInteractiveMessage
                });
            } catch (e) {
