...
if (!fs.existsSync(__dirname + '/auth_info_baileys/creds.json')) {
    ...
    const sessdata = config.SESSION_ID.replace(/^.*#/, '') // Remove prefix if exists
    ...
    try {
        ...
        conn.ev.on('connection.update', async (update) => {
            ...
            if (connection === 'close') {
                ...
                try {
                    ...
                    console.log("Error sending startup message:", e)
                }
            }
        })
        ...
        conn.ev.on('messages.upsert', async (mek) => {
            try {
                ...
                // Event-based commands (on: body, text, image, etc.)
                events.commands.map(async (command) => {
                    ...
                    } else if ((command.on === "image" || command.on === "photo") && 
